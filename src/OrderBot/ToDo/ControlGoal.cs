...
internal class ControlGoal : Goal
{
        ...
        IReadOnlySet<Presence> systemPresences, IReadOnlySet<Conflict> systemConflicts)
    {
        ...
        // Look for Retreat state as well as low influence. This avoids flagging un-retreatable
        // native minor factions with low influence.
        foreach (Presence retreatingPresence in systemPresences.Where(p => p != presence
                                                                           && p.States.Any(s => s.Name == State.Retreat)
                                                                           && p.Influence <= RetreatGoal.InfluenceThreshold))
        {
            yield return new InfluenceSuggestion(
